...
<br/>
## Q 15.0. Methods in a promise ?

There are 6 methods in the promise as mentioned:-

1. `Promise.all(iterable):` Returns a single Promise that resolves when all of the promises in the iterable argument have resolved or when the iterable contains no promises.
...
3. `Promise.any(iterable):` Takes an iterable of Promise objects and, as soon as one of the promises in the iterable fulfills, returns a single promise that resolves with the value from that promise.
...
5. `Promise.resolve(value):` Returns a Promise object that is resolved with the given value. If the value is a promise, that promise is returned; if the value is a thenable (i.e., has a "then" method), the returned promise will "follow" that thenable, adopting its eventual state; otherwise, the returned promise will be fulfilled with the value.

6. `Promise.reject(reason):` Returns a Promise object that is rejected with the given reason.

These methods provide powerful utilities for managing asynchronous operations and handling promises in JavaScript. Each serves a different purpose and can be used depending on the specific requirements of your code.



## Q 15.1. What is a promise?

`Promise is an asynchronous Code`
...
Imagine a case when implementing the `promise.all and any 1 promise got rejected` then How will the `Promise.all` act:-

Certainly! If one of the promises in the Promise.all() call gets rejected, the Promise.all() call itself will reject immediately with the reason of the first promise that rejects. Here's an example:

# IMPORTANT POINTS
1. The fulfillment value of the new promise is an array with the fulfillment values of the original promises in the same order as the original promises.
2. If any of the passed-in promises reject, the Promise.all() call immediately rejects with the reason of the first promise that rejects, discarding all the other promises whether they've resolved or not.
...
Promise.all([promise1, promise2, promise3])
  .then((values) => {
    console.log(values); // This will not execute
  })
  .catch((error) => {
...
1. promise1 resolves with a value of 10.
2. promise2 is rejected with the reason "Error: Promise rejected".
3. promise3 resolves with a value of 30 after 1 second.
4. Since one of the promises (promise2) is rejected, Promise.all() immediately rejects with the reason of promise2.
5. The .then() block is skipped, and the .catch() block catches the rejection reason, which is "Error: Promise rejected", and logs it to the console.
